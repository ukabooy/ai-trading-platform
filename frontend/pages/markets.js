            ...
            {tickers.map((t) => (
              <div key={t.symbol} style={styles.row}>
                <div style={styles.coinInfo}>
                  <div style={styles.coinIcon}>{t.symbol.replace('USDT', '').slice(0, 3)}</div>
                  <span style={styles.symbol}>{t.symbol.replace('USDT', '')}</span>
                </div>
                <div style={styles.priceInfo}>
                  <span style={styles.price}>
                    ${t.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: t.price > 100 ? 2 : 6 })}
                  </span>
                  <span style={{
                    ...styles.change,
...
const styles = {
  ...
  row: {
    ...
    background: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '10px', padding: '12px 14px',
  },
  ...
  priceInfo: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end' },
